...
static class Handover1Class : public TclClass {
public:
  Handover1Class() : TclClass("Agent/MIHUser/IFMNGMT/MIPV6/Handover/Handover1") {}
  TclObject* create(int, const char*const*) {
    return (new Handover1());
  }
 ...
 */
Handover1::Handover1() : Handover(), scanTimer_(0), connectingMac_(-1)
{
  bind ("case_", &case_);
  bind ("confidence_th_", &confidence_th_); 
}

void Handover1::register_mih ()
{
 ...
 * @param e The event information
 */
void Handover1::process_link_up (link_up_t *e)
{
  ...
  debug ("At %f in %s Handover1 received link up \n\ttype %d, MacAddr=%d, MacPoA=%d\n", \
	 NOW, Address::instance().print_nodeaddr(addr()),\
	 e->linkIdentifier.type,  e->linkIdentifier.macMobileTerminal, e->linkIdentifier.macPoA);
 ...
 * @param e The event information
 */
void Handover1::process_link_down (link_down_t *e)
{
 ...
 * @param e The event information
 */
void Handover1::process_link_going_down (link_going_down_t *e)
{
  ...
  debug ("At %f in %s Handover1 received Link going down \n\t probability = %d%%\n", \
	 NOW, Address::instance().print_nodeaddr(addr()),\
	 e->confidenceLevel);
 ...
 */
void Handover1::process_exp_prefix (exp_prefix* data)
{
  Mac *mac;
  Tcl& tcl = Tcl::instance();

  debug ("At %f in %s Handover1 received expired prefix %s\n", \
	 NOW, Address::instance().print_nodeaddr(addr()),\
 ...
 */
void Handover1::process_cap_disc_conf (hdr_mih *mihh)
{
  ...
  char *str1 = Address::instance().print_nodeaddr(this->addr());
  debug ("At %f in %s Handover1 received MIH_Capability_discover.confirm\n", NOW, str1);
  delete []str1; 
 ...
 */
void Handover1::process_register_conf (struct mih_reg_rsp *rsp)
{
  assert (rsp!=NULL);

  char *str1 = Address::instance().print_nodeaddr(this->addr());
  debug ("At %f in %s Handover1 received MIH_Register.confirm\n", NOW, str1);
  delete []str1;
  debug ("\tStatus is : %d\n", rsp->reg_result);
}
 ...
 */
void Handover1::process_deregister_conf (struct mih_dereg_rsp *rsp)
{
  assert (rsp!=NULL);

  char *str1 = Address::instance().print_nodeaddr(this->addr());
  debug ("At %f in %s Handover1 received MIH_Deregister.confirm\n", NOW, str1);
  delete []str1;
  debug ("\tStatus is : %d\n", rsp->result_code);
}
 ...
 */
void Handover1::process_scan_conf (struct mih_scan_rsp *rsp)
{
  bool found = false;
  BSSDescription *n;
  ...
  if (!found) {
	...
	scanTimer_ = new ScanTimer (this, linkIdentifier);
...
void 
Handover1::process_scanTimer (struct link_identifier_t linkIdentifier)
{
  //scan 802.11
